...
$(document).ready(function () {
  ...
  $('[type="button"]').click(function () {
    ...
    for (const i in listof) {
      amen.amenities.push(i);
      console.log('amenities_______________' + amen.amenities);
    }
    console.log('Amen______' + amen);
    $.ajax({
      type: 'POST',
      ...
      success: function (data) {
        ...
        $.each(data, function (key, place) {
          ...
          const bath = $('<div class="number_bathrooms">' + place.number_bathrooms + ' Bathroom' + (place.number_bathrooms > 1 ? 's' : '') + '</\
div>');
